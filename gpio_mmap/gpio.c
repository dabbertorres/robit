...
#define GPIO_SET_OFF (0x1C)
#define GPIO_CLR_OFF (0x28)
#define GPIO_LVL_OFF (0x34)
...
int gpio_init(gpio_dir_map map)
{
    ...
    gpio_reg = (volatile uint32_t*)mmap(0, getpagesize(), PROT_READ | PROT_WRITE, MAP_SHARED, fd, GPIO_ADDR);
...
int gpio_deinit()
{
    int res = munmap((void*)gpio_reg, getpagesize());
